    ...
    (function () {
      ...
      // Reduced from 120 to 50 particles for performance
        ...
        for (let i = 0; i < particles.length; i++) {
          for (let j = i + 1; j < particles.length; j++) {
            ...
            if (dist2 < maxDist2) {
              ...
              ctx.strokeStyle = `rgba(0,245,255,${0.04 * (1 - dist / maxDist)})`;
              ctx.lineWidth = 0.5;
              ctx.stroke();
            }
          }
    ...
    if (canvas) {
      ...
      function initCanvas() {
        ...
        particles = [];
        // Capped at 80 for smooth performance
        const numParticles = Math.min((width * height) / 12000, 80);
      ...
      function animateCanvas() {
        ...
        for (let i = 0; i < particles.length; i++) {
          ...
          // Draw particle (no shadowBlur — huge GPU savings)
          ctx.beginPath();
          ...
          // Use squared distance to skip sqrt when not needed
          for (let j = i + 1; j < particles.length; j++) {
            let p2 = particles[j];
            let dx = p.x - p2.x;
            let dy = p.y - p2.y;
            ...
            if (dist2 < connDist2) {
              let distance = Math.sqrt(dist2);
              ctx.beginPath();
              ctx.strokeStyle = `rgba(191, 0, 255, ${0.3 * (1 - distance / connDist)})`;
              ctx.lineWidth = 0.5;
              ctx.moveTo(p.x, p.y);
    ...
    /* ===== IDE SCRIPTS ===== */
    (function () {
      const files = ['neural_restore.py', 'stocksense.py', 'civic_system.py', 'smart_attendance.py', 'driver_monitor.py'];
    ...
    (function initGlobe() {
      ...
      var items = [
        ...
        { n: 'PyTorch', img: 'https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pytorch/pytorch-original.svg', s: 36 },
      ...
      // Throttled to ~30fps for better CPU usage
      var lastGlobeFrame = 0;
      function animate(timestamp) {
        requestAnimationFrame(animate);
        if (timestamp - lastGlobeFrame < 33) return; // ~30fps
    ...
    const colors_sgpa = ['#00f5ff', '#ff0080', '#bf00ff', '#00ff88', '#ffee00', '#ff00ff'];
    ...
    <div class="sgpa-item">
      ...
      <div class="sgpa-bar-wrap">
        <div class="sgpa-bar" data-pct="${pct}" style="background: linear-gradient(90deg, ${colors_sgpa[i % colors_sgpa.length]}, ${colors_sgpa[(i + 1) % colors_sgpa.length]});"></div>
      </div>
    ...
    window.filterProjects = function(category, btnElement) {
      ...
      const items = document.querySelectorAll('.featured-project-item');
      items.forEach(item => {
  ...
  Education: B.Tech Information Technology, MBIT Anand Gujarat, CGPA 8.61/10, Expected May 2027.
  Semester SGPAs: S1:8.83, S2:8.17, S3:8.48, S4:8.30, S5:8.75, S6:9.19
  Skills: Python, SQL, HTML5, CSS3, JavaScript, NumPy, Pandas, Matplotlib, Streamlit, Scikit-learn, MediaPipe, TensorFlow, OpenCV, Flask, Git, GitHub, SQLite, PostgreSQL, YOLOv11, XGBoost, GARCH, Plotly, PyTorch
  Projects:
  1. NeuralRestore — AI-Powered Image Restoration (2026) - Sequential 5-model deep learning pipeline (EfficientNet-B4 + UNet, LaMa, DnCNN, NAFNet, Real-ESRGAN 4x) with Gradio UI, test-time augmentation, offline execution.
  2. Driver Drowsiness & Distraction Detection System (March 2026) - Real-time dual-phase system using Face Mesh for EAR-based drowsiness detection, head pose estimation, auto-calibration, 8-frame smoothing. Custom YOLOv11 trained on 2004 images for phone detection.
  3. StockSense AI (2025) - AI stock chatbot with WhatsApp-style UI, natural language queries, RSI/MACD/Bollinger Bands. Deployed on Render.com.
  4. Anand Civic Issue Reporting System (2025) - Role-based platform (4 roles) with GPS boundary validation, Gemini AI image analysis, JWT auth. Deployed on Render.com.
  5. Smart Attendance System (2024) - Face recognition web-based attendance with role-based dashboards, CSV/PDF export, custom C++ compile script.
